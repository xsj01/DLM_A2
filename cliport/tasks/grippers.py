...
# with initialize(config_path="../../ndf/cfg"):
#     cfg = compose(config_name="config")
# if cfg.transparent_robot:
#     SPATULA_BASE_URDF = 'ur5/spatula/spatula-base-transparent.urdf'
# else:
#     SPATULA_BASE_URDF = 'ur5/spatula/spatula-base.urdf'
SPATULA_BASE_URDF_TRANSPARENT = 'ur5/spatula/spatula-base-transparent.urdf'
SPATULA_BASE_URDF = 'ur5/spatula/spatula-base.urdf'
SPATULA_LONG_URDF = 'ur5/spatula/spatula-long.urdf'
SPATULA_SHORT_URDF = 'ur5/spatula/spatula-short.urdf'
SPATULA_CYLINDER_URDF = 'ur5/spatula/spatula-cylinder.urdf'
SPATULA_CYLINDER_URDF_TRANSPARENT = 'ur5/spatula/spatula-cylinder-transparent.urdf'
...
class Spatula(Gripper):
    ...
    def __init__(self, assets_root, robot, ee, obj_ids, cfg=None):  # pylint: disable=unused-argument
        ...
        if cfg and cfg.get('pusher') != 'normal':
            if cfg.get('pusher')=='long':
                base = pybullet_utils.load_urdf(
                p, os.path.join(self.assets_root, SPATULA_LONG_URDF), pose[0], pose[1])
            elif cfg.get('pusher')=='short':
                base = pybullet_utils.load_urdf(
                p, os.path.join(self.assets_root, SPATULA_SHORT_URDF), pose[0], pose[1])
            elif cfg.get('pusher')=='cylinder':
                if cfg.get("transparent"):
                    base = pybullet_utils.load_urdf(
                    p, os.path.join(self.assets_root, SPATULA_CYLINDER_URDF_TRANSPARENT), pose[0], pose[1]) 
                else:
                    base = pybullet_utils.load_urdf(
                    p, os.path.join(self.assets_root, SPATULA_CYLINDER_URDF), pose[0], pose[1])
        elif cfg and cfg.get("transparent"):
            base = pybullet_utils.load_urdf(
                p, os.path.join(self.assets_root, SPATULA_BASE_URDF_TRANSPARENT), pose[0], pose[1])
        else:
            base = pybullet_utils.load_urdf(
