...
# if cfg.transparent_robot:
    # UR5_URDF_PATH_TRANSPARENT = 'ur5/ur5_transparent.urdf'
# else:
...
class Environment(gym.Env):
    ...
    def __init__(self,
        ...
        if (disp is False) and self.cfg.get("enableGPU"):
    ...
    def reset(self, object_poses=None):
        ...
        if self.cfg.get("transparent"):
            self.ur5 = pybullet_utils.load_urdf(
                p, os.path.join(self.assets_root, UR5_URDF_PATH_TRANSPARENT))
        else:
            self.ur5 = pybullet_utils.load_urdf(
                p, os.path.join(self.assets_root, UR5_URDF_PATH))
        self.ee = self.task.ee(self.assets_root, self.ur5, 9, self.obj_ids, self.cfg)
        ...
        # Reset task.
        self.task.reset(self, object_poses)
    ...
    def movej(self, targj, speed=0.01, timeout=10):
        ...
        while (time.time() - t0) < timeout:
            ...
            # if "transporter" in self.cfg and self.cfg["transporter"]:
            #     pass
            # else:
            gains[-1] *=1.5
    ...
    def solve_ik(self, pose):
        ...
        # print('1',joints)
        if pose[0][-1]<0.1:
            joints[2:-1] = (joints[2:-1] + np.pi) % (2 * np.pi) - np.pi
        else:
            joints[2:] = (joints[2:] + np.pi) % (2 * np.pi) - np.pi
        # print('2',joints)
        # print('diff', joints-joints0)
        return joints
...
class PilesEnvironment(Environment):
    def step(self, action=None):
        """Execute action with specified primitive.
